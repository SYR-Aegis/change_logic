    ...
    def change_english(change_list):  # 만약 처음이라면 끝이라면 등등 고려해야 되네
        ...
        for keyword in change_list:
            ...
            if keyword == 'a' or keyword == 'A':  # 라파한테 줄때는 mov input_value, a_en이렇게 하자
                ...
                return_list.append(destination)
                english_fini = 1
                check_special_character(keyword) #숫자 같은 경우에는 영어의 끝을 찍고 수표를 찍어야 하네 
...
#################################################################################################
    ...
    def find_hangul(print_list):
        nonlocal destination
        nonlocal return_list
        nonlocal english_continue
        young_index = 0
        change_yeong_index = 0
        change_yeong_value = 0
        check_index_number = 1 # 숫자였냐 한글이였나 판단한다.
        #초기값을 1로 해야겠다. "2개" 일떄 처리할려면? 
        length_l = len(print_list)
        for i, keyword in enumerate(print_list):  # 만약 이 예외처리 조건에 부합하지 않는다면 분리시키고 각 각 함수로 넣는다.
            if ord(keyword) >= 0x41 and ord(keyword) <= 0x7A:
                if english_continue == 0:
                    destination = start_en  
                    return_list.append(destination)
                    change_english(keyword)
                    english_continue = 1
                    if i == length_l:
                        destination = end_en
                        return_list.append(destination)
...
#################################################################################################
                ...
                elif ord(letter[i + 1][0]) >= 0xAC00 and ord(letter[i + 1][0]) <= 0xD7A3:
                    if english_fini == 0:
                        destination = end_en
                        return_list.append(destination)
                    else:
                ...
                elif ord(letter[i + 1][0]) >= 32 and ord(letter[i + 1][0]) <= 64:
                    if english_fini == 0:
                        destination = end_en
                        return_list.append(destination)
                    else:
